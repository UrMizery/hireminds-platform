...
const PLACEHOLDER_REFERRAL_CODES = ["PATHWAY2026", "SKILLSQUEST2026"];
...
export default function SkillsQuestPage() {
  ...
  useEffect(() => {
    async function checkAccess() {
      ...
      if (partnerResult.error) {
        console.error(
          "Career Pathway partner access check failed:",
          partnerResult.error
        );
      }
      ...
      if (candidateResult.error) {
        console.error(
          "Career Pathway candidate access check failed:",
          candidateResult.error
        );
      }

      const accountType = String(partnerResult.data?.account_type || "")
        .trim()
        .toLowerCase();
  ...
  const activeDays = trainingDays.filter(
    (day) => day.modules?.length
  ).length;
  ...
  return (
    <main style={styles.main}>
      <section style={styles.hero}>
        <p style={styles.kicker}>
          HireMinds Career Pathway
        </p>

        <h1 style={styles.title}>
          3-Week Career Pathway Program
        </h1>
        ...
        <div style={styles.overviewGrid}>
          ...
          <div style={styles.overviewCard}>
            ...
            <span>
              assessment-based days complete
            </span>
          </div>
        </div>
      </section>
      ...
      <section style={styles.weekStack}>
        {["Week 1", "Week 2", "Week 3"].map((week) => {
          const info =
            weekDetails[
              week as keyof typeof weekDetails
            ];
