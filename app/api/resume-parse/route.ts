...
export async function POST(req: NextRequest) {
  try {
    if (!process.env.ANTHROPIC_API_KEY) {
      return NextResponse.json(
        { error: "Resume parser is not configured." },
        { status: 500 }
      );
    }
...
Never invent, improve, infer, or rewrite facts.
...
${resumeText.slice(0, 18000)}`;
    ...
    try {
      ...
      return NextResponse.json(
        {
          error:
            "The resume was read, but its structure could not be parsed. Please try again.",
        },
        { status: 502 }
      );
    }
    ...
    return NextResponse.json({
      ...
      // IMPORTANT:
      // Job Match Analyzer needs the actual extracted resume text.
      resumeText,

      // Keep structured data for Resume Generator and other HireMinds tools.
      parsedResume,
